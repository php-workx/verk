...
func TestReviewFindingBlocks(t *testing.T) {
	...
	cases := []struct {
		...
		// Expired waiver: WaiverExpiresAt is in the past → treated as open → blocks.
		{
			name: "waived with expired WaiverExpiresAt blocks",
			finding: state.ReviewFinding{
				ID:              "f-4",
				Severity:        state.SeverityP1,
				...
				WaivedBy:        "reviewer",
				WaivedAt:        now,
				WaiverReason:    "temporary waiver",
				WaiverExpiresAt: now.Add(-24 * time.Hour),
			},
			threshold: state.SeverityP1,
			want:      true,
		},
		// Future waiver: WaiverExpiresAt is ahead → still valid → does not block.
		{
			name: "waived with future WaiverExpiresAt does not block",
			finding: state.ReviewFinding{
				ID:              "f-5",
				Severity:        state.SeverityP1,
				Title:           "future waiver",
				Body:            "waiver is still valid",
				File:            "internal/engine/closeout.go",
				Line:            10,
				Disposition:     "waived",
				WaivedBy:        "reviewer",
		...
		// Zero WaiverExpiresAt: no expiry set → waiver is perpetual → does not block.
		{
			name: "waived with zero WaiverExpiresAt does not block",
			finding: state.ReviewFinding{
				ID:           "f-6",
				Severity:     state.SeverityP1,
				Title:        "perpetual waiver",
				Body:         "waiver has no expiry",
				File:         "internal/engine/closeout.go",
				Line:         10,
				Disposition:  "waived",
				WaivedBy:     "reviewer",
		...
		// Open finding with no waiver fields → blocks at threshold (regression guard).
		{
			name: "open with no waiver fields blocks",
			finding: state.ReviewFinding{
				ID:          "f-7",
				Severity:    state.SeverityP1,
				Title:       "open finding",
				Body:        "open finding body",
				File:        "internal/engine/closeout.go",
				Line:        10,
				Disposition: "open",
			},
